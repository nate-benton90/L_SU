...
sub get_incompatibles {
	...
	my $params = {
		_two_needed		=> @_two_needed,
		_one_needed		=> @_one_needed,
	...
	@only_one 				= 	('pbal','mbal','agc','tpow','epow','etpow','gpow');	
	push $params->{_one_needed}	,	\@only_one;
	
	print("sugain, get_incomatpibles  @{$params->{_one_needed}} \n");
	
#	$only_one[1]           		= 	('tpow','jon');
#	push $params->{_one_needed}	,	\$only_one[1];
#	
#	$only_one[2]           		= 	('gpow','jon');
#	push $params->{_one_needed}	,	\$only_one[2];
#		
#	$only_one[3]           		= 	('qclip','jon');
#	push $params->{_one_needed}	,	\$only_one[3];
#	
#	$needed[0]    				= 	('agc','wagc');
#	push $params->{_two_needed}	, 	\$needed[0];
#	
#	$needed[1]    				= 	('agc','gagc');
#	push $params->{_two_needed}	, 	\$needed[1];
...
=cut
 ...
 sub tpower {
	...
	if ( $tpower ) {
		...
		$sugain->{_tpower}		= $tpower;
		$sugain->{_note}		= $sugain->{_note}.' tpow='.$sugain->{_tpower};
		$sugain->{_Step}		= $sugain->{_Step}.' tpow='.$sugain->{_tpower};
