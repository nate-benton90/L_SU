...
sub test {
 my ($test,@value) = @_;
 print("\$test or the first_file_number scalar  'holds' a  HASH $test 
...
sub first_file_number {
    ...
    $sucat->{_first_file_number} = $first_file_number if defined($first_file_number);
}
...
sub inbound_directory {
    ...
    $sucat->{_inbound_directory} = $inbound_directory if defined($inbound_directory);
}
...
sub last_file_number {
    ...
    $sucat->{_last_file_number} = $last_file_number if defined($last_file_number);
...
=cut
 ...
 sub Step {
	...
	$sucat->{_Step}   = $sucat->{_Step}.$sucat->{_inbound_directory}.'/'.$sucat->{_list_array}[$i].$sucat->{_input_suffix}.' \\'.$newline;
        #print(" list is $sucat->{_Step}\n\n");
       }
    } elsif($sucat->{_first_file_number} < $sucat->{_last_file_number}) {
     		for ( my $file=$sucat->{_first_file_number}; $file <= $sucat->{_last_file_number}; $file++) {
       			$sucat->{_Step} = $sucat->{_Step}.' '.
			 $sucat->{_inbound_directory}.'/'.$file.$sucat->{_input_suffix}; 
     		}
    } elsif ($sucat->{_first_file_number} > $sucat->{_last_file_number}) {
		print( "sucat, Step, reverse order assumed\n");
        	for (my $file=$sucat->{_first_file_number}; $file >= $sucat->{_last_file_number}; $file=($file-1)) {
                	$sucat->{_Step} = $sucat->{_Step}.' '.
                        $sucat->{_inbound_directory}.'/'.$file.$sucat->{_input_suffix};
