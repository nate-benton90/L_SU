...
=cut
 ...
 use Project;
 use SuMessages;
 use SeismicUnix qw ($go $in $on $off $ibot_mute $itemp_bot_mute_picks_ $itemp_bot_mute_num_points $itemp_bot_mute_picks_sorted_par_ $ibot_mute_par_ $suffix_su $suffix_hyphen $to);
...
=cut
 ...
 sub clear {
     ...
     $iApply_bottom_mute->{_binheader_type} 	= '';
...
sub binheader_type{
   ...
   $iApply_bottom_mute->{_binheader_type} 	= $binheader_type if defined($binheader_type);
   #print(" header type is $iApply_bottom_mute->{_binheader_type}\n\n");
}
...
sub offset_type{
   ...
   $iApply_bottom_mute->{_offset_type} 	= $offset_type if defined($offset_type);
...
sub file_in {
    ...
    $iApply_bottom_mute->{_inbound}	  	= $DATA_SEISMIC_SU.'/'.$iApply_bottom_mute->{_file_in}.$suffix_su;
 }
...
sub  min_amplitude {
 my ($variable, $min_amplitude)         = @_;
    $iApply_bottom_mute->{_min_amplitude}        = $min_amplitude  if defined($min_amplitude);
...
sub  max_amplitude {
 my ($variable, $max_amplitude)         = @_;
    $iApply_bottom_mute->{_max_amplitude}        = $max_amplitude  if defined($max_amplitude);
...
$iApply_bottom_mute->{_parfile_in} = $itemp_bot_mute_picks_sorted_par_.$iApply_bottom_mute->{_file_in};

 #print("mute pick file is $iApply_bottom_mute->{_parfile_in}\n\n");
...
=cut
 ...
 $suwind   		-> clear();
 $suwind   		-> setheaderword($iApply_bottom_mute->{_binheader_type});
...
=cut
 ...
 $base_caption	  = $iApply_bottom_mute->{_file_in}.quotemeta(' f=').$iApply_bottom_mute->{_freq};
 $windowtitle	  = $iApply_bottom_mute->{_binheader_type}.quotemeta(' = ').$iApply_bottom_mute->{_gather_num};
