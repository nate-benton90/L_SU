...
=cut 
  ...
  use SeismicUnix qw ($true $false $in $out $to $ibot_mute_par_ $itemp_bot_mute_picks_ $itemp_bot_mute_picks_sorted_par_  $out $suffix_su);
...
sub type {
   my($variable,$type)  = @_;
   $iBottomMutePicks2par3->{_type}   = $iBottomMutePicks2par3 if defined($iBottomMutePicks2par3);
}
...
=cut
  ...
  $mkparfile_out = $itemp_bot_mute_picks_sorted_par_.$iBottomMutePicks2par3->{_file_in};
...
#   print("Regular xt data\n\n");
 ...
 @items   = ($mkparfile[1],$in,$iBottomMutePicks2par3->{TX_inbound},$out,$iBottomMutePicks2par3->{TX_outbound});
