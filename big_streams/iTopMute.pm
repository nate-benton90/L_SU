...
sub binheader_type{
   ...
   $iTopMute->{_binheader_type} 	= $binheader_type if defined($binheader_type);
}
...
sub iSelect_tr_Sumute_top{
 ...
 $iSelect_tr_Sumute_top->binheader_type($iTopMute->{_binheader_type});
...
sub iApply_top_mute{
 ...
 $iApply_top_mute->binheader_type($iTopMute->{_binheader_type});
...
sub iSave_top_mute_picks{
 ...
 $iSave_top_mute_picks->binheader_type($iTopMute->{_binheader_type});
...
=cut
   ...
   $SuMessages->binheader_type($iTopMute->{_binheader_type}); 
...
sub number_of_tries {
    ...
    $iTopMute->{_number_of_tries} 		= $number_of_tries if defined($number_of_tries); 
...
# sort file names
  ...
  $inbound	  			= $PL_SEISMIC.'/'.$sortfile_in.'_'.$iTopMute->{_file_in}.'_'.$iTopMute->{_gather_num_suffix};

# TX write file names
  $writefile_out 		= $XTpicks_out;
  $outbound  		    = $PL_SEISMIC.'/'.$writefile_out.'_'.$iTopMute->{_file_in}.'_'.$iTopMute->{_gather_num_suffix};
