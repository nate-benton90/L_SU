...
=cut 
  ...
  use SuMessages;
  use SeismicUnix qw ($on $off $in $to $go $itop_mute_par_ $itop_mute_check_pickfile_ $false $true );
...
sub binheader_type{
   ...
   $iTop_Mute3->{_binheader_type} 	= $binheader_type if defined($binheader_type);
}
...
sub iSelect_tr_Sumute_top{
 ...
 $iSelect_tr_Sumute_top->binheader_type($iTop_Mute3->{_binheader_type});
...
sub iApply_top_mute{
 ...
 $iApply_top_mute->binheader_type($iTop_Mute3->{_binheader_type});
...
sub iSave_top_mute_picks{
 ...
 $iSave_top_mute_picks->binheader_type($iTop_Mute3->{_binheader_type});
...
=cut
   ...
   $SuMessages->binheader_type($iTop_Mute3->{_binheader_type}); 
...
sub number_of_tries {
    ...
    $iTop_Mute3->{_number_of_tries} 		= $number_of_tries if defined($number_of_tries); 
...
# sort file names
  ...
  $inbound	  		= $PL_SEISMIC.'/'.$sortfile_in.'_'.$iTop_Mute3->{_file_in}.'_'.$iTop_Mute3->{_gather_num_suffix};

# TX write file names
  $writefile_out 		= $XTpicks_out;
  $outbound  		        = $PL_SEISMIC.'/'.$writefile_out.'_'.$iTop_Mute3->{_file_in}.'_'.$iTop_Mute3->{_gather_num_suffix};
