...
=cut 
  ...
  use SeismicUnix qw ($true $false $in $out $to $itop_mute_par_ $itemp_top_mute_picks_ $itemp_top_mute_picks_sorted_par_  $out $suffix_su);
...
sub type {
   my($variable,$type)  = @_;
   $iTopMutePicks2par3->{_type}   = $iTopMutePicks2par3 if defined($iTopMutePicks2par3);
}
...
=cut
  ...
  $mkparfile_out = $itemp_top_mute_picks_sorted_par_.$iTopMutePicks2par3->{_file_in};
...
#   print("Regular xt data\n\n");
 ...
 @items   = ($mkparfile[1],$in,$iTopMutePicks2par3->{TX_inbound},$out,$iTopMutePicks2par3->{TX_outbound});
