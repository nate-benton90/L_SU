...
=cut
 ...
 use SeismicUnix qw ($go $in $out $on $off $itop_mute $itemp_top_mute_picks_ $itemp_top_mute_num_points $itemp_top_mute_picks_sorted_par_ $itop_mute_par_ $suffix_su $suffix_hyphen $to);
=head2
...
sub binheader_type{
   ...
   $iApply_mute->{_binheader_type} 	= $binheader_type if defined($binheader_type);
   #print(" header type is $iApply_mute->{_binheader_type}\n\n");
}
...
sub file_in {
    ...
    print(" file in is $iApply_mute->{_file_in} in iApply_Mute\n\n");
       $iApply_mute->{_inbound}	  	= $DATA_SEISMIC_SU.'/'.
					  $iApply_mute->{_file_in}.'_'.
					  $iApply_mute->{_gather_type}.
					  $iApply_mute->{_gather_num}.
					  $suffix_su;

       $iApply_mute->{_outbound}	= $DATA_SEISMIC_SU.'/'.
					  $iApply_mute->{_file_in}.'_'.
					  $iApply_mute->{_gather_type}.
					  $iApply_mute->{_gather_num}.
					  '_mute'.$suffix_su;
...
sub  min_amplitude {
 my ($variable, $min_amplitude)         = @_;
    $iApply_mute->{_min_amplitude}        = $min_amplitude  if defined($min_amplitude);
...
sub  max_amplitude {
 my ($variable, $max_amplitude)         = @_;
    $iApply_mute->{_max_amplitude}        = $max_amplitude  if defined($max_amplitude);
