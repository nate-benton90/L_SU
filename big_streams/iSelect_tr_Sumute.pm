...
=cut
 ...
 sub clear {
     ...
     $iSelect_tr_Sumute->{_binheader_type} = '';
...
sub file_in {
   ...
   if (defined($file_in) && $iSelect_tr_Sumute->{_gather_type} 
		          && $iSelect_tr_Sumute->{_gather_num}) {
       $iSelect_tr_Sumute->{_file_in} 	= $file_in if defined($file_in); 
       $iSelect_tr_Sumute->{_inbound}	= $DATA_SEISMIC_SU.'/'.$iSelect_tr_Sumute->{_file_in}.'_'.$iSelect_tr_Sumute->{_gather_type}.$iSelect_tr_Sumute->{_gather_num}.$suffix_su;
    }
    else {
...
sub binheader_type{
   my($variable,$type)  	= @_;
   if (defined($type)) {
     $iSelect_tr_Sumute->{_binheader_type} 	= $type ;
   }
}
...
sub  min_amplitude {
 my ($variable, $min_amplitude)         = @_;
    $iSelect_tr_Sumute->{_min_amplitude}        = $min_amplitude  if defined($min_amplitude);
...
sub  max_amplitude {
 my ($variable, $max_amplitude)         = @_;
    $iSelect_tr_Sumute->{_max_amplitude}        = $max_amplitude  if defined($max_amplitude);
...
sub number_of_tries {
  my ($variable, $number_of_tries)        = @_;
     $iSelect_tr_Sumute->{_number_of_tries}        = $number_of_tries if defined($number_of_tries);
...
sub suxwigb_defaults {
  my ($variable, $suxwigb_defaults)        = @_;
     $iSelect_tr_Sumute->{_suxwigb_defaults}        = $suxwigb_defaults if defined($suxwigb_defaults);
