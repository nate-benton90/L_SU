...
=cut
	...
	sub get_values {
		...
		$config_superflows  ->set_program_name($synseis->{_prog_name});		 
 		...
 		my $plot_density_max  				= @{$synseis->{_values_aref}}[6]; 
 		my $plot_density_min  				= @{$synseis->{_values_aref}}[7]; 
 		my $plot_depth_min  				= @{$synseis->{_values_aref}}[8];  
 		my $plot_depth_max  				= @{$synseis->{_values_aref}}[9]; 
 		...
 		my $plot_reflection_coefficient_max = @{$synseis->{_values_aref}}[15]; 
 			 ...
 			 print(" 1. readfiles,config, for synseis, base_file_name is $base_file_name\n\n");
  		$base_file_name 						= $control->su_data_name(\$base_file_name);
			 print(" 2. readfiles,config, for synseis, base_file_name is $base_file_name \n\n");
		...
		my $CFG_h = {
    		synseis => {
        		...
        		plot_depth_min 					=> $plot_depth_min,
        		plot_density_max 				=> $plot_density_max,
        		plot_depth_min					=> $plot_depth_min,
        		plot_depth_max					=> $plot_depth_max,
        		plot_time_min_s 				=> $plot_time_min_s,
        		plot_time_max_s 				=> $plot_time_max_s,
        		plot_reflection_coefficient_min	=> $plot_reflection_coefficient_min,
        		plot_reflection_coefficient_max	=> $plot_reflection_coefficient_max,
         	}      
    	}
 	}; 
 	
 	return($CFG_h,$synseis->{_values_aref}); # hash and arrary reference
