...
=cut
 ...
 sub clear {
     ...
     $iApply_top_mute3->{_binheader_type}   	= '';
...
sub binheader_type{
   ...
   $iApply_top_mute3->{_binheader_type} 	= $binheader_type if defined($binheader_type);
   #print(" header type is $iApply_top_mute3->{_binheader_type}\n\n");
}
...
sub file_in {
    ...
    $iApply_top_mute3->{_inbound}	  	= $DATA_SEISMIC_SU.'/'.$iApply_top_mute3->{_file_in}.$suffix_su;
 }
...
sub  min_amplitude {
 my ($variable, $min_amplitude)         = @_;
    $iApply_top_mute3->{_min_amplitude}        = $min_amplitude  if defined($min_amplitude);
...
sub  max_amplitude {
 my ($variable, $max_amplitude)         = @_;
    $iApply_top_mute3->{_max_amplitude}        = $max_amplitude  if defined($max_amplitude);
...
$iApply_top_mute3->{_parfile_in} = $itemp_top_mute_picks_sorted_par_.$iApply_top_mute3->{_file_in};
...
=cut
 ...
 $suwind   		-> clear();
 $suwind   		-> setheaderword($iApply_top_mute3->{_binheader_type});
...
=cut
 ...
 $base_caption	  = $iApply_top_mute3->{_file_in}.quotemeta(' f=').$iApply_top_mute3->{_freq};
 $windowtitle	  = $iApply_top_mute3->{_binheader_type}.quotemeta(' = ').$iApply_top_mute3->{_gather_num};
...
#for suximage
 @items   = ($suwind[1],$in,$iApply_top_mute3->{_inbound},$to,$suwind[2],$to,$sumute[1],$to,$sugain[2],$to,$suximage[1],$go);
  $flow[1] = $run->modules(\@items);

#for suxwigb 
 @items   = ($suwind[1],$in,$iApply_top_mute3->{_inbound},$to,$suwind[2],$to,$sumute[1],$to,$sugain[2],$to,$suxwigb[1],$go);
