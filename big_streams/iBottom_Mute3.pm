...
sub binheader_type{
   ...
   $iBottom_Mute3->{_binheader_type} 	= $binheader_type if defined($binheader_type);
}
...
sub iSelect_tr_Sumute_bottom{
 ...
 $iSelect_tr_Sumute_bottom->binheader_type($iBottom_Mute3->{_binheader_type});
 ...
 $iSelect_tr_Sumute_bottom->min_amplitude($iBottom_Mute3->{_min_amplitude});
 $iSelect_tr_Sumute_bottom->max_amplitude($iBottom_Mute3->{_max_amplitude});
 $iSelect_tr_Sumute_bottom->number_of_tries($iBottom_Mute3->{_number_of_tries});
...
sub iApply_bottom_mute{
 ...
 $iApply_bottom_mute->binheader_type($iBottom_Mute3->{_binheader_type});
...
sub iSave_bottom_mute_picks{
 ...
 $iSave_bottom_mute_picks->binheader_type($iBottom_Mute3->{_binheader_type});
...
=cut
   ...
   $SuMessages->binheader_type($iBottom_Mute3->{_binheader_type}); 
...
sub type {
  ...
  $iBottom_Mute3->{_exists} = $check4old_data->type($iBottom_Mute3->{_type});
...
sub gather_num {
   ...
   $iBottom_Mute3->{_gather_num_suffix}     = '_gather'.$iBottom_Mute3->{_gather_num}
...
sub  max_amplitude {
 my ($variable, $max_amplitude)         = @_;
    $iBottom_Mute3->{_max_amplitude}        = $max_amplitude  if defined($max_amplitude);
...
sub  min_amplitude {
 my ($variable, $min_amplitude)         = @_;
    $iBottom_Mute3->{_min_amplitude}        = $min_amplitude       if defined($min_amplitude);
...
sub number_of_tries {
    ...
    $iBottom_Mute3->{_number_of_tries} 		= $number_of_tries if defined($number_of_tries); 
...
# sort file names
  ...
  $inbound	  		= $PL_SEISMIC.'/'.$sortfile_in.'_'.$iBottom_Mute3->{_file_in}.'_'.$iBottom_Mute3->{_gather_num_suffix};

# TX write file names
  $writefile_out 		= $XTpicks_out;
  $outbound  		        = $PL_SEISMIC.'/'.$writefile_out.'_'.$iBottom_Mute3->{_file_in}.'_'.$iBottom_Mute3->{_gather_num_suffix};
