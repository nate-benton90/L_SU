...
sub number_of_tries {
  my ($variable, $number_of_tries)         = @_;
     $iSuvelan->{_number_of_tries}          = $number_of_tries if defined($number_of_tries);
...
sub velocity_increment {
    ...
    $iSuvelan->{_velocity_increment} 		= $velocity_increment if defined($velocity_increment); 
...
=cut
 ...
 $suvelan     -> clear();
 $suvelan     -> number_of_velocities($iSuvelan->{_number_of_velocities});
 $suvelan     -> velocity_increment(($iSuvelan->{_velocity_increment} * $iSuvelan->{_data_scale}));
 $suvelan     -> first_velocity(($iSuvelan->{_first_velocity} * $iSuvelan->{_data_scale}));
...
=cut
 ...
 $N=2;
   $windowtitle[1]		 ='\('.$N.'\)\ '.$iSuvelan->{_sufile_in}.'\ CDP=\ '.$iSuvelan->{_cdp_num};
 #units=Semblance					\\
 $suximage -> clear();
 ...
 $suximage -> box_X0(600);    
 $suximage -> box_Y0(0);    
 $suximage -> title($iSuvelan->{_cdp_num_suffix});    
 $suximage -> windowtitle('Semblance');    
 $suximage -> ylabel("TWTT-s");    
 $suximage -> xlabel("Velocity-m/s");    
 $suximage -> legend($on);    
 $suximage -> cmap('hsv2');    
 $suximage -> first_x($iSuvelan->{_first_velocity});
 $suximage -> dx($iSuvelan->{_velocity_increment});
 #$suximage -> dt_s($new_dt_s);
 $suximage -> loclip($iSuvelan->{_min_semblance});
 $suximage -> hiclip($iSuvelan->{_max_semblance});
 $suximage -> verbose($off);
#print ("d2num is ($iSuvelan->{_velocity_increment})\n\n");
 $suximage -> dx_major_divisions(($iSuvelan->{_velocity_increment})*10);
 ...
 $suximage -> first_tick_number_x($iSuvelan->{_first_velocity});
  $suximage -> picks(\$iSuvelan->{_Tvel_outbound});
 #print("Writing picks to $iSuvelan->{_Tvel_outbound}\n\n");
...
=cut
 ...
 if ($iSuvelan->{_number_of_tries} >= 2 ) {
  ...
  $suximage    ->curvefile(\$iSuvelan->{_Tvel_inbound});
  my ($ref_T_nmo,$ref_Vnmo,$num_tvel_pairs) = manage_files_by::read_2cols(\$iSuvelan->{_Tvel_inbound});
  $suximage     ->npair($num_tvel_pairs);
  $suximage     ->curvecolor(2);
} 
...
=cut
 ...
 $suxwigb-> clear(); 
 $suxwigb-> title($iSuvelan->{_sufile_in}); 
 $suxwigb-> ylabel(quotemeta('Time s'));  
 $suxwigb-> xlabel(quotemeta('Offset m')); 
 $suxwigb-> box_width(300); 
 $suxwigb-> box_height(450); 
 $suxwigb-> box_X0(875); 
 $suxwigb-> box_Y0(0); 
 $suxwigb-> absclip(2);
 $suxwigb-> xcur(1);
 $suxwigb-> windowtitle($iSuvelan->{_cdp_num_suffix});
 $suxwigb-> shading(1);
...
sub  number_of_velocities{
    ...
    $iSuvelan->{_number_of_velocities} 		= $number_of_velocities if defined $number_of_velocities; 
