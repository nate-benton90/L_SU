...
# cat file names
   $catfile_inbound[1]		= $PL_SEISMIC.'/'.$cpfile_in[1].'_'.$sufile_in[1].$suffix[1];
   $storefile_outbound[1]	= $PL_SEISMIC.'/'.$cpfile_out[1].'_'.$sufile_in[1].$suffix[1];

 $textfile_in[1]		= 'ivpicks_old'.'_'.$iWrite_All_iva_out->{_file_in}.$suffix[1];

  $duplicatefile_in[1]		= 'ivpicks_'.$iWrite_All_iva_out->{_file_in}.$suffix[1];

  $Tvel_duplicate_inbound[1]	= $PL_SEISMIC.'/'.$duplicatefile_in[1];

 $duplicatefile_out[1]		= 'ivpicks_old'.'_'.$iWrite_All_iva_out->{_file_in}.$suffix[1];
