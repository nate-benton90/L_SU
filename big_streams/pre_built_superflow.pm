 ...
 sub select {
	...
	#$param_widgets		->set_labels_w_aref($superflow->{_labels_w_aref} );
 	...
 	$param_widgets		->set_current_program($prog_name_sref);
 		...
 		# program name that is selected (i.e. through spec.pm... if you do it in param widgets)
	...
	$Entry_widget[0]->focus;  
 	...
 	# in order to export this private hash we need to send if back via a private vairble
