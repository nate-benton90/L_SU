...
=cut
 ...
 use SeismicUnix qw ($in $out $on $go $to $off $suffix_su $iSpectralAnalysisPickFile); 
 use flow;
 use message;
 use readfiles; 
 use sufilter;
...
=cut
	...
	my ($CFG_h,$CFG_aref) 		= $iSpectralAnalysis_config->get_values();
  	...
  	my $max_amplitude 			= $CFG_h->{suxwigb}{1}{max_amplitude};
  	my $headerWord 				= $CFG_h->{suxwigb}{1}{headerWord};
  	my $min_amplitude_phase 	= $CFG_h->{suxwigb}{1}{min_amplitude_phase};
  	my $max_amplitude_freq 		= $CFG_h->{suxwigb}{1}{max_amplitude_freq};
...
=cut
 ...
 $analyze      ->absclip_phase($min_amplitude_phase);
 $analyze      ->absclip_freq($max_amplitude_freq);
