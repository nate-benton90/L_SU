...
=cut
 ...
 sub clear {
     ...
     $iSelect_tr_Sumute_top3->{_binheader_type} = '';
...
sub gather_num {
   ...
   $iSelect_tr_Sumute_top3->{_gather_num} 		= $gather_num if defined ($gather_num);
}
...
sub binheader_type{
   my($variable,$type)  	= @_;
   $iSelect_tr_Sumute_top3->{_binheader_type} 	= $type if defined($type);
}
...
sub file_in {
  ...
  $iSelect_tr_Sumute_top3->{_inbound}	  		= $DATA_SEISMIC_SU.'/'.$iSelect_tr_Sumute_top3->{_file_in}.$suffix_su;
...
sub  min_amplitude {
 my ($variable, $min_amplitude)         = @_;
    $iSelect_tr_Sumute_top3->{_min_amplitude}        = $min_amplitude  if defined($min_amplitude);
...
sub  max_amplitude {
 my ($variable, $max_amplitude)         = @_;
    $iSelect_tr_Sumute_top3->{_max_amplitude}        = $max_amplitude  if defined($max_amplitude);
...
sub number_of_tries {
  my ($variable, $number_of_tries)        = @_;
     $iSelect_tr_Sumute_top3->{_number_of_tries}        = $number_of_tries if defined($number_of_tries);
...
=cut
 ...
 $suwind   		-> clear();
 $suwind   		-> setheaderword($iSelect_tr_Sumute_top3->{_binheader_type});
...
#plotting with suximage 
 $base_caption[1]	  = $iSelect_tr_Sumute_top3->{_file_in}.quotemeta('  ').quotemeta('f=').$iSelect_tr_Sumute_top3->{_freq};

 $windowtitle[1]	  = quotemeta('GATHER = ').$iSelect_tr_Sumute_top3->{_gather_num};
...
if ($iSelect_tr_Sumute_top3->{_number_of_tries} > 0) {
  $iSelect_tr_Sumute_top3->{_TX_outbound} = $itemp_top_mute_picks_.$iSelect_tr_Sumute_top3->{_file_in};
...
$base_caption[2]	  = $iSelect_tr_Sumute_top3->{_file_in}.quotemeta('  ').quotemeta('f=').$iSelect_tr_Sumute_top3->{_freq};

$windowtitle[2]	  = quotemeta('GATHER = ').$iSelect_tr_Sumute_top3->{_gather_num};
...
#for suximage
 @items   = ($suwind[1],$in,$iSelect_tr_Sumute_top3->{_inbound},$to,$suwind[2],$to,$sufilter[1],$to,$sugain[2],$to,$suximage[1],$go);
   $flow[1] = $run->modules(\@items);

#for suxwigb 
 @items   = ($suwind[1],$in,$iSelect_tr_Sumute_top3->{_inbound},$to,$suwind[2],$to,$sufilter[1],$to,$sugain[2],$to,$suxwigb[1],$go);
