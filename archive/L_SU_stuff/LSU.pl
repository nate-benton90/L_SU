...
=cut 
	...
	# for all
 	...
 	# L_SU::messages::message_director is used in the following methods
 	# check_code, save_button, FileDialog_button, File_dailog_close, add2flow_button
 	# delete_from_flow  sunix_select run_button	
	...
	# 1. user_built_flow.pm 2. built_in_flow.pm
		# 1.for flow_select save_button delete_from_flow_button
		# FileDialog_close
	use decisions 1.00;
	
	# for FileDialog_close, FileDialog_button
	use iFile;
	
	# 1. user_built_flow.pm 2. built_in_flow.pm
		# for run_button FileDialog_close
	use control;
	...
	# 1.user_built_flow.pm 2.built_in_flow.pm
		...
		# _change4checek, check_code_button save_button
	use param_flow;
	
	
	# for 2.built_in_flow.pm only
	use config_superflows;
		# used in the following methods: sunix_select drop add2flow FileDialog_button
		# delete_from_flow * could potentially be used in superflow package
		
	# FileDialogButton add2flow_button
	use whereami;
	...
	use flow_widgets;
		# used in sunix_select add2flow_button
	use param_sunix;
	
	
	# for 1.user_built_flow.pm 2.built_in_flow.pm for 3. PlTk.pm
	# $TkPl_SU->{_message_box_w} 
	
	# refactoring in progress:
	use user_built_flows;
	my @user_built_flows_pm;
	my $grey = 0;
	$user_built_flows_pm[$grey]	= user_built_flows->new;
...
=cut
 	...
 	my ($sunix_listbox_w,$flow_listbox_l,$flow_listbox_r,$flow_listbox_sw,$flow_listbox_se);
...
=cut
	...
	my $Tk_LSU->{_file_menubutton_w} 		= $file_menubutton;
...
=cut
 	...
 	$TkPl_SU->{_run_button_w}					= 	$run_button;
...
=cut
 	...
 	$sunix_listbox_w 	= $sunix_frame->Scrolled(
		       	   			"Listbox", 
                       	   -scrollbars 		=> "osoe", 
                       	   -height 			=> 20, 
 	...
 	$sunix_listbox_w    ->insert(
                       "end", 
                        sort @choices
	             		);
	             		
	$TkPl_SU->{_sunix_listbox_w}		= $sunix_listbox_w;
...
=cut
 	...
 	$sunix_listbox_w	-> bind('<1>' 
			=> [\&sunix_select]     
                       	);
 	$sunix_listbox_w	-> bind('<3>' 
			=> [\&help]     
                       	);
...
=cut
 	...
 	$TkPl_SU->{_message_box_w} 	= $message;
 	 # test $TkPl_SU->{_message_box_w}    ->insert('end', "This is some normal text\n");
...
=cut
    ...
    $TkPl_SU->{_flow_listbox_grey_w} = $flow_listbox_l;
...
sub _built_in_flows {
	...
	use Moose;
	# require built_in_flows;
	
	# my $built_in_flow[0] = built_in_flow->new();
	# built_in_flows->$set_method();
	
	# find out which flow is current
	# calls increase_vigil_on_delete_counter
...
=head2 sub user_built_flows
	...
	whole class needs widgets: $TkPl_SU
	methods:
			set a message area
	
	call same method for each flow color
	$color = 0 = $grey;
...
sub _user_built_flows {
	...
	use Moose;
		...
		# $user_built_flows_pm[$color]->set_message_area($TkPl_SU);	
		# $user_built_flows_pm[$color]->$method($TkPl_SU);
...
TODO: Better encapsulation
	...
	calls
		...
		_conditions   -> set4FileDialog_select_end();
	sees:
		messages box
...
sub FileDialog_button {
	...
	my $message          	= $L_SU_messages->null_button(0);
 	$TkPl_SU->{_message_box_w}	->delete("1.0",'end');
 	$TkPl_SU->{_message_box_w}	->insert('end', $message);
	...
	if ($$option_sref eq 'Open') {
   		_conditions  -> set4FileDialog_open_start();
		$widget_type		= $whereami->widget_type($parameter_values_frame);
   		...
   		_conditions   -> set4FileDialog_open_end();
	}
							# Only flows can be saved
	if ($$option_sref eq 'SaveAs') {
   		_conditions   -> set4FileDialog_saveas_start();
    	...
    	if($TkPl_SU->{_is_flow_listbox_l} 	|| 
   		...
   		_conditions   -> set4FileDialog_saveas_end();
	}
...
sub FileDialog_close {
	...
	if ($option eq  'Select') {
		...
		my $pre_req_ok 		= $decisions->get4FileDialog_select();
   		...
   		#_conditions->set4FileDialog_select_start(); #needed?
		} # if prereq_OK
	}
				...
				if($suffix ne '') {
					...
					my $message = $L_SU_messages->FileDialog_close(2);
 	    			...
 	    			$TkPl_SU->{_message_box_w}    ->insert('end', $message);
   					...
   					#_conditions->set4FileDialog_open_start(); #needed?
	} 				# if open
					# superflows and flows are not allowed to open perl files 
...
sub drag {
  	my ($self) = @_;   
					#print("1. main,drag;just starting\n");
					#print("1. main,drag;listbox can not be empty\n");
					#print("1. main,drag;more than one item must exist\n");
  my $prog_name;
  if($TkPl_SU->{_prog_name}) {
	$prog_name					= ${$TkPl_SU->{_prog_name}};
	#print("1. main,drag,prog=$prog_name\n");
   }
 	...
 	if( $prog_name && $flow_widgets->is_drag_deleted($flow_listbox_l) ) {

		print("2.main,drag; prior index was deleted\n");
...
# in param_flow->set_box_number(4);
 ...
 sub set_new_flow {	
 	my (@self,$hash_ref) = @_;	
 	my $TkPl_SU = $hash_ref;
 	_set_prog_versions($TkPl_SU);
	privat_set_prog_names;
	_set_param_values_aref2($TkPl_SU->{_values_aref});
    _set_param_names_aref2($TkPl_SU->{_names_aref});
	_set_param_checkbuttons_aref2($TkPl_SU->{_check_buttons_settings_aref});
	return();
 }
...
sub add2flow_button {
	...
	_conditions->set4start_of_add2flow_button();
   	...
   	$sunix_listbox_w->selectionClear($sunix_listbox_w->curselection);
...
=head2 sub _stack_flow
  ...
  print("main,_stack_flow,last left listbox flow program touched had index = $TkPl_SU->{_last_flow_index_touched}\n");
  print("main,_stack_flow,values= @{$TkPl_SU->{_values_aref}}\n");
...
=head2 sub _stack_superflow
  ...
  the initial version comes from default parameter files
  Using the same code as for sunix_select
						 print("main,_stack_superflow,last left listbox flow program touched had index = $TkPl_SU->{_last_flow_index_touched}\n");
...
=head2 sub delete_from_flow_button
 ...
 call:
    stack_versions();	
	_conditions->set4last_delete_from_flow_button();
    _conditions->set4delete_from_flow_button();
...
=head2 sub sunix_select
  ...
  call:
    _conditions			->set4start_of_sunix_select;
    _conditions	->set4end_of_sunix_select() ;
...
sub sunix_select {
    ...
    _conditions				->set4start_of_sunix_select;
   	...
   	my $here 				= $whereami->get4sunix_listbox();
        ...
        # get program name
   	$TkPl_SU->{_prog_name} 				= $param_widgets->get_current_program(\$sunix_listbox_w);
...
sub _check4changes {
	...
	if ($TkPl_SU->{_last_flow_index_touched} >= 0) {  # <-1 does exist
		...
		my $last_idx_chng =$TkPl_SU->{_last_flow_index_touched} ;
	 					      ...
	 					      #  print("main,_check4changes,
	 					      # last changed entry index was $last_idx_chng\n");
  							  # print("main, _check4changes program name is 
  							  # ${$TkPl_SU->{_prog_name}}\n");
		...
		$TkPl_SU->{_check_buttons_settings_aref}= $param_widgets	->get_check_buttons_aref();
  							...
  							# print("main,_changes4changes,changed names_aref: @{$TkPl_SU->{_names_aref}}\n");
  ...
  TODO: Encapsulation is poor:
    ...
    TkPl_SU
 	...
 	calls
 	  	stack_versions
  		_check4changes();
 		_conditions->set4start_of_flow_select(); 
  		_conditions->set4end_of_flow_select(); 
...
sub flow_select {
	...
	my $message          		= $L_SU_messages->null_button(0);
 	...
 	$TkPl_SU->{_message_box_w}	->insert('end', $message);
				...
				#  TODO place which listbox (left or right) was chosen in _conditions
	...
	if ($pre_req_ok) { 
		...
		_check4changes();
 		...
 		$TkPl_SU->{_names_aref} 	= $param_flow->get_names_aref();
  							...
  							# print("2. main, flow_select,parameter names: @{$TkPl_SU->{_names_aref}}\n");
 		$TkPl_SU->{_values_aref} 	=	$param_flow->get_values_aref();

  				 			# print("3. main, flow_select,parameter values:@{$TkPl_SU->{_values_aref}}\n");
 		$TkPl_SU->{_check_buttons_settings_aref} 
									=  $param_flow->get_check_buttons_settings();

  				 			# print("4. main, flow_select, check_buttons_settings no changes, @{$TkPl_SU->{_check_buttons_settings_aref}}\n");
  				 			# print("4. main, flow_select,index;$index\n#");
...
=head2 sub run_button
 ...
 TODO: Encapsulate better
  ...
  calls 
	_conditions		->set4run_button_start();
	_conditions		->set4run_button();     
	$decisions		      	->set4run_select($TkPl_SU);
	_conditions		->set4run_button_end();
	$whereami				->in_gui();
	message box: 			$TkPl_SU->{_message_box_w}
...
sub superflow_select {
	...
	_conditions			->set4superflow_select();
...
sub check_code_button {
	...
	_conditions	->set4start_of_check_code_button();
...
# 	  	$TkPl_SU->{_message_box_w}->insert('end', $message);
	...
	_conditions	->set4end_of_check_code_button();
   	return();
}
...
DB:
 ...
 i/p: $L_SU_messages
 i/p: $TkPl_SU->{_message_box_w}
 i/p: $param_flow
 i/p: $TkPl_SU
 i/p: $config_superflows
 
 o/p: _conditions	->set4start_of_save_button();
 o/p: _conditions	->set4_save_button
 o/p: _conditions	->set4end_of_save_button();
 o/p: _check4changes();
...
sub save_button {
	...
	_conditions	->set4start_of_save_button();
	...
	_conditions	->set4end_of_save_button();
   	return();
}
...
sub set_flow_listbox_touched {
	...
	# TODO in future the program must keep track of which listbox was just chosen
   	$TkPl_SU->{_last_flow_listbox_touched} 		= 'flow_listbox_l';
    		  # print("main,_last set_flow_listbox_touched left listbox = $TkPl_SU->{_last_flow_listbox_touched}\n");
...
sub set_flow_listbox_touched_w {
	...
	# TODO in future the program must keep track of which listbox was just chosen
   	$TkPl_SU->{_last_flow_listbox_touched_w} 	= $flow_listbox_l;	
    		  # print("main,_last, set_flow_listbox_touched_w= $TkPl_SU->{_last_flow_listbox_touched_w}\n");
...
sub set_flow_index_touched {
	my ($self) = @_;
   		...
   		$TkPl_SU->{_last_flow_index_touched} 	= $flow_widgets->get_flow_selection($flow_listbox_l);
							  # print("1. main,_last,last left listbox flow program touched had index = $TkPl_SU->{_last_flow_index_touched}\n");
...
sub  set4FileDialog_select_start {
	my $self = @_;
	#_conditions->reset();
	 #  ERROR if _conditions->reset() the param_widgets table is reset;
	 #  ERROR if _conditions->reset() the 	 #  f 
    $TkPl_SU->{_is_select_file_button}		= $true;
    # print("main,_conditions,set4FileDialog_select_start  $TkPl_SU->{_is_select_file_button} 	\n");
    #print("main,_conditions,set4FileDialog_open_start,listbox_l listbox_r  $TkPl_SU->{_is_flow_listbox_l} 	$TkPl_SU->{_is_flow_listbox_r}\n");
...
sub  set4FileDialog_select_end {
	my $self = @_;
    $TkPl_SU->{_is_select_file_button}		= $false;
    # print("main,_conditions,set4FileDialog_select_end  $TkPl_SU->{_is_select_file_button}\n");
    #print("main,_conditions,set4FileDialog_open_start,listbox_l listbox_r  $TkPl_SU->{_is_flow_listbox_l} 	$TkPl_SU->{_is_flow_listbox_r}\n");
...
sub  set4FileDialog_open_start {
    ...
    # print("main,_conditions,set4FileDialog_open_start  $TkPl_SU->{_is_open_file_button}\n");
...
sub  set4FileDialog_open_end {
	my $self = @_;
    $TkPl_SU->{_is_open_file_button}		= $false;
    # print("main,_conditions,set4FileDialog_open_end  $TkPl_SU->{_is_open_file_button}\n");
...
sub  set4FileDialog_saveas_start {
	my $self = @_;
	#_conditions->reset();
	 					#  ERROR if _conditions->reset() 
	 					#  the param_widgets table is reset; 	 
	 					
    $TkPl_SU->{_is_saveas_file_button}		= $true;
    					# print("main,_conditions,set4FileDialog_saveas_start  
...
sub  set4FileDialog_saveas_end {
    ...
    # $TkPl_SU->{_path}						= '';
    					...
    					# print("main,_conditions,set4FileDialog_saveas_end  
    					# $TkPl_SU->{_is_saveas_file_button}\n");
	return();
}
...
sub set4start_of_check_code_button {
	my $self = @_;
	# _conditions	->reset();
				  # print("1. main,_configure,set4start_of_button,last left listbox flow program touched had index = $TkPl_SU->{_last_flow_index_touched}\n");
        		# location within GUI   
...
sub set4_check_code_button {
	my $self = @_;
	# _conditions	->reset();
				  # print("1. main,_configure,set4end_of_check_code_button,last left listbox flow program touched had index = $TkPl_SU->{_last_flow_index_touched}\n");
        		# location within GUI   
...
sub set4end_of_flow_select {
	my $self = @_;
	_conditions	->reset();
	_last			->set_flow_index_touched();
				  # print("1. main,_configure,set4end_of_flow_select,last left listbox flow program touched had index = $TkPl_SU->{_last_flow_index_touched}\n");
        		# location within GUI   
...
sub set4_save_button {
	my $self = @_;
	# _conditions	->reset();
				  # print("1. main,_configure,set4end_of_save_button,last left listbox flow program touched had index = $TkPl_SU->{_last_flow_index_touched}\n");
        		# location within GUI   
...
sub set4start_of_flow_select {
	my $self = @_;
	_conditions	->reset();
	_last			->set_flow_listbox_touched();
    _last			->set_flow_listbox_touched_w();
        		# location within GUI   
   	$TkPl_SU->{_is_flow_listbox_l}			= $true;
   	$TkPl_SU->{_is_flow_listbox_r}			= $true;
