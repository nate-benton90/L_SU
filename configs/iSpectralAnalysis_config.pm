...
=cut
 ...
 sub get_values {
	...
	$config_superflows  ->set_program_name($iSpectralAnalysis_config->{_prog_name});
        
     					# parameter values from superflow configuration file
   	$iSpectralAnalysis_config->{_values_aref} = $config_superflows->get_values();
	...
	my $sugain_1_agc_gain_width			= @{$iSpectralAnalysis_config->{_values_aref}}[2];
 	my $suxwigb_1_max_amplitude_freq  	= @{$iSpectralAnalysis_config->{_values_aref}}[3]; 
 	my $suxwigb_1_headerWord  			= @{$iSpectralAnalysis_config->{_values_aref}}[4]; 
 	my $suxwigb_2_min_amplitude_phase	= @{$iSpectralAnalysis_config->{_values_aref}}[5];
 	my $suxwigb_3_max_amplitude_freq  	= @{$iSpectralAnalysis_config->{_values_aref}}[6]; 
...
=cut
 ...
 my $CFG = {
    ...
    sugain  => {
  	  	1 => {
        	agc_gain_width	=> $sugain_1_agc_gain_width,
        },      
    },
    suxwigb  => {
  	  	1  => {
         ...
         2 => {
        	min_amplitude_phase => $suxwigb_2_min_amplitude_phase,
         },
         3 => {
        	max_amplitude_freq   => $suxwigb_3_max_amplitude_freq,
         }
    }, 
    base_file_name => $base_file_name,
 }; # end of CFG hash 
 
 	return( $CFG,$iSpectralAnalysis_config->{_values_aref} ); # hash and arrary reference
