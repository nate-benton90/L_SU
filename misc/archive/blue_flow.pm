...
=cut
 	...
 	sub set_hash_ref {
 		...
 		$dnd_token				= $hash_ref->{_dnd_token_blue};# K
 		$dropsite_token			= $hash_ref->{_dropsite_token_blue};# K
 		...
 		$userBuiltFlow->{_dnd_token}					= $hash_ref->{_dnd_token_blue}; # K
 		$userBuiltFlow->{_dropsite_token}				= $hash_ref->{_dropsite_token_blue}; # K
		...
		$userBuiltFlow->{_message_w}					= $hash_ref->{_message_w}; 
 		...
 		$userBuiltFlow->{_values_w_aref} 				= $hash_ref->{_values_w_aref};
 		 ...
 		 # print("blue_flowset_hash_ref,delete_from_flow_button: $delete_from_flow_button\n");
 		 # print("blue_flowset_hash_ref,parameter_values_frame: $userBuiltFlow->{_parameter_values_frame}\n");
 		 # print("1. blue_flowset_hash_ref,_message_w: $userBuiltFlow->{_message_w}\n");
 		 # print("blue_flowset_hash_ref,parameter_values_frame: $parameter_values_frame\n"); 	
 		 # print("blue_flowset_hash_ref,add2flow_button_grey,$add2flow_button_grey	\n");
 		 # print("blue_flow,set_hash_ref,userBuiltFlow->{_flow_color}: $userBuiltFlow->{_flow_color}	\n");
 		 # print("blue_flowset_hash_ref,delete_from_flow_button: $delete_from_flow_button\n");
 		 # print("blue_flow,set_hash_ref,flow_color: $flow_color\n");
 		return();
 	}
...
=cut
 	...
 	sub get_flow_color {
 		...
 		if( $userBuiltFlow->{_flow_color}) {
 			...
 			print("blue_flow, missing flow color\n");
 		}
...
sub drag {
  	my ($self) = @_;   
					# print("1. blue_flowdrag;just starting\n");
					# print("1. blue_flowdrag;listbox can not be empty\n");
					# print("1. blue_flowdrag;more than one item must exist\n");
  my $prog_name;
  if($userBuiltFlow->{_prog_name_sref}) {
	$prog_name					= ${$userBuiltFlow->{_prog_name_sref}};
	# print("1. blue_flowdrag,prog=$prog_name\n");
   }
 	...
 	if( $prog_name && $flow_widgets->is_drag_deleted($flow_listbox_blue_w) ) {
			...
			print("2.blue_flowdrag; prior index was deleted\n");
        my $this_index = $flow_widgets->get_drag_deleted_index($flow_listbox_blue_w);  # K

			print("blue_flowdrag,deleting flow_listbox,idx=$this_index\n");
	...
	my $num_items = $flow_widgets->get_num_items($flow_listbox_blue_w); # K
  						# print("blue_flowdrag,num items prior to deletion  = $num_items\n");

  	if ( $num_items > 1 ) {  # num_items PRIOR to deletion
   	   $flow_widgets->drag_start($dnd_token);
	    				# print("blue_flowdrag,start dnd_token:$dnd_token\n");
  	} else {
						# print("WARNING: blue_flowdrag,not allowed\n");
  	}						 
}
     ...
     $userBuiltFlow->{_destination_index} 	= $flow_widgets->destination_index;
							...
							# note the last program that was touched
    						 $userBuiltFlow->{_last_flow_index_touched} 	= $userBuiltFlow->{_destination_index} ;
							 	# print(" blue_flowdrop, destination index = $userBuiltFlow->{_last_flow_index_touched}\n");
...
sub increase_vigil_on_delete_counter {
	...
	# print("blue_flowincrease_vigil_on_delete_counter\n");
	return();
}
 ...
 sub _set_flow_listbox_color_w {
	...
	if ($flow_color eq 'grey') {
		$userBuiltFlow->{_flow_listbox_color_w} =  $userBuiltFlow->{_flow_listbox_grey_w};
			#print("blue_flow,_set_flow_listbox_color_w, _set_flow_listbox_color_w, color is $flow_color\n");
		...
		$userBuiltFlow->{_flow_listbox_color_w} =  $userBuiltFlow->{_flow_listbox_blue_w};
			#print("blue_flow,set_flow_listbox_color_w, _set_flow_listbox_color_w, color is $flow_color\n");		
	} else {
		print("blue_flow,_set_flow_listbox_color_w, _set_flow_listbox_color_w, missing color\n");
	}
...
=head2 sub add2flow_button
  		...
  		foreach my $key (sort keys %$userBuiltFlow) {
   			print (" blue_flow key is $key, value is $userBuiltFlow->{$key}\n");
  		}
...
sub add2flow_button {
	...
	$userBuiltFlow->{_flow_type} = $flow_type->{_user_built};
	 	# print("1. blue_flow, add2flow_button, color: $flow_color \n");
	 	# print("1. blue_flow, add2flow_button, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");
	...
	$conditions_gui				-> set4start_of_add2flow_button($flow_color);
	 	# print("2. blue_flow, add2flow_button, color: $flow_color \n"); 	
    ...
    # print("3. blue_flow, add2flow_button, _prog_name_sref: ${$userBuiltFlow->{_prog_name_sref}}\n"); 
    # print("2. blue_flow, add2flow_button, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");
    ...
    _set_flow_listbox_color_w($flow_color);
   			# print("blue_flow,add2flow_button,userBuiltFlow->listbox_widget: $userBuiltFlow->{_flow_listbox_color_w}\n");
   	$userBuiltFlow->{_flow_listbox_color_w} ->insert(
                       	"end", 
                       	${$userBuiltFlow->{_prog_name_sref}},
	               );
	  		#print("3. blue_flow, add2flow_button, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");
	  		#print("3. blue_flow, add2flow_button, _labels_w_aref $userBuiltFlow->{_labels_w_aref}\n");
   	...
   	$userBuiltFlow->{_param_sunix_length}  			= $param_sunix->length(); # # values not index 
	 	# print("4. blue_flow, add2flow_button, color: $flow_color \n"); 
   	$whereami			->set4add2flow_button();
   		 	# print("5. blue_flow, add2flow_button, color: $flow_color \n");
	...
	$param_widgets		->set_labels_w_aref($userBuiltFlow->{_labels_w_aref} );
  	...
  	print("2. blue_flow, add2flow_button, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");
   	...
   	$param_widgets		->redisplay_check_buttons();
   		  	# print("3. blue_flow, add2flow_button, _labels_w_aref $userBuiltFlow->{_labels_w_aref}\n");
	  		# print("4.  blue_flow, add2flow_button, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");
   ...
   _stack_versions();
   	...
   	_stack_flow();
	  		# print("5. blue_flow, add2flow_button, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");   	
	...
	my $index 				= $flow_widgets->get_flow_selection($userBuiltFlow->{_flow_listbox_color_w});
							# print("blue_flowadd2flow_button,last left flow program touched had index: $index\n");
											# print("66. blue_flow, add2flow_button, color: $flow_color \n");
	$conditions_gui			-> set_flow_index_last_touched($index); # flow color is not reset
	$userBuiltFlow 			=  $conditions_gui->get_hash_ref();
				# print("66. blue_flow, add2flow_button, color: $flow_color \n");
	   			# print("2. blue_flow,add2flow_button,userBuiltFlow->listbox_widget: $userBuiltFlow->{_flow_listbox_color_w}\n");
		  		print("6. blue_flow, add2flow_button, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");
	flow_select();
	
   			# print("3. blue_flow,add2flow_button,userBuiltFlow->listbox_widget: $userBuiltFlow->{_flow_listbox_color_w}\n");
...
=head2 sub _stack_flow
  ...
  print("blue_flow_stack_flow,last left listbox flow program touched had index = $userBuiltFlow->{_last_flow_index_touched}\n");
  print("blue_flow_stack_flow,values= @{$userBuiltFlow->{_values_aref}}\n");
...
sub _stack_flow {
  my( $self) = @_;
    	print("1. blue_flow, _stack_flow, B4 num_items\n");
  		my $num_items = $param_flow->get_num_items();
  		# print("blue_flow,_stack_flow,userBuiltFlow->listbox_widget: $userBuiltFlow->{_flow_listbox_color_w}\n");

  		# print("blue_flow, _stack_flow, color: $flow_color \n");
  ...
  $param_flow		->stack_checkbuttons_aref2($userBuiltFlow->{_check_buttons_settings_aref});	
	    ...
	    print("2. blue_flow, _stack_flow, After num_items: $num_items\n");			 
...
=head2 sub delete_from_flow_button
 ...
 call:
  ...
  $userBuiltFlow 			= $conditions_gui->get_hash_ref();
  		...
  		foreach my $key (sort keys %$userBuiltFlow) {
   			print (" blue_flowkey is $key, value is $userBuiltFlow->{$key}\n");
  		}
...
sub delete_from_flow_button {
	...
	my $self = @_;
 	...
 	my $flow_color			= $conditions_gui->get_flow_color();
 		# print("1. blue_flow,delete_from_flow_button, flow_color: $flow_color \n");
 	
 	if ($flow_color)  {
 		_set_flow_color($flow_color);
 					# print("2. blue_flow,delete_from_flow_button, flow_color: $userBuiltFlow->{_flow_color} \n");
		use messages::message_director;
		use decisions 1.00;
		...
		my $message          	= $userBuiltFlow_messages->null_button(0);
 		...
 		my $flow_listbox_color_w = _get_flow_listbox_color_w();
					# print("1. blue_flow,delete_from_flow_button, flow_listbox_color_w: $flow_listbox_color_w \n");
		$userBuiltFlow 			= $conditions_gui->get_hash_ref(); # find out if delete button is active
					# print("1. blue_flow,delete_from_flow_button , is_flow_listbox_grey pink green or blue_w: $userBuiltFlow->{_is_flow_listbox_color_w} \n");
		...
		my $pre_req_ok 			= $decisions->get4delete_from_flow_button();
					...
					# print("1. blue_flow,delete_from_flow_button pre_req $pre_req_ok\n");
  		if ($pre_req_ok) {
					 # print("2. blue_flowdelete_from_flow_button pre_req_ok\n");
			...
			if ($index == 0 && $param_flow->get_num_items == 1) {
     			...
     			$flow_widgets->delete_selection($flow_listbox_color_w);
   				...
   				$userBuiltFlow->{_last_flow_index_touched} 	= -1;
							...
							#  print("blue_flowdelete_from_flow_button,
  				...
  				my $index2delete 	= $flow_widgets->get_index2delete();
  					 		# print("blue_flowdelete_from_stored,index2delete:$index2delete\n");
   				...
   				$param_widgets		->gui_clean();
							...
							# note the last program that was touched
							 $userBuiltFlow->{_last_flow_index_touched} = $index;
					   		...
					   		# print("blue_flow_delete_from_flow_button, last index touched :$index..\n");
     			$flow_widgets->delete_selection($flow_listbox_color_w);

					# delete stored programs and their parameters
   					# delete_from_stored_flows(); 
  				my $index2delete 	= $flow_widgets->get_index2delete();
  					 		#print("2. blue_flowdelete_from_stored,index2delete:$index2delete\n");
  				$param_flow		->delete_selection($index2delete);
            	...
            	if($next_idx_selected_after_deletion == -1) { $next_idx_selected_after_deletion = 0} # NOT < 0
  					  		# print("2. blue_flowdelete_from_flow_button,indexafter_deletion:
 				...
 				$userBuiltFlow->{_names_aref} 	= $param_flow->get_names_aref();
  							 ...
  							 # print("2. blue_flow delete_from_flow_button,parameter names 
  							 # is @{$userBuiltFlow->{_names_aref}}\n");
 				$userBuiltFlow->{_values_aref} 	=	$param_flow->get_values_aref();

  				 			 # print("3. blue_flow delete_from_flow_button,parameter values 
  				 			 # is @{$userBuiltFlow->{_values_aref}}\n");
 				$userBuiltFlow->{_check_buttons_settings_aref} 
										=  $param_flow->get_check_buttons_settings();

  				 			 # print("4. blue_flow delete_from_flow_button, 
				...
				$here 							= $whereami->get4flow_listbox();
				 			 # print("blue_flow delete_from_flow_button, $here->{_is_flow_listbox_color_w}\n");
	    		...
	    		$param_widgets      		->set_location_in_gui($here);
 				...
 				$param_widgets				->set_check_buttons(
					  		 ...
					  		 # print("blue_flow, delete_from_flow_button, is listbox selected:$here->{_is_flow_listbox_color_w}\n");
 				$param_widgets				->redisplay_labels();
 				$param_widgets				->redisplay_values();
 					$param_widgets			->redisplay_check_buttons();
	#  			$param_widgets				->set_entry_change_status($false);	
 		...
 		print("blue_flow, delete_from_flow_button, flow color missing: \n");
 	}
}
 ...
 sub _move_in_stored_flows {
	...
	$userBuiltFlow->{_index2move}        	= $flow_widgets->index2move();
  	...
  	my $end	    = $userBuiltFlow->{_destination_index};
   			 # print("blue_flow_move_in_stored_flows,start index is the $start\n");
   			 # print("blue_flow_move_in_stored_flows, insertion index is $end \n");
...
=cut 
 ...
 sub _get_flow_listbox_color_w {
	...
	if ($flow_color eq 'grey') {
		...
		print("blue_flow, _get_flow_listbox_color_w, neutral color\n");
	} else {
		print("blue_flow, _get_flow_listbox_color_w, missing color\n");
...
=cut 
 ...
 sub _set_flow_color {
	...
	if ($flow_color) {
		# print("blue_flow _set_flow_color , color:$flow_color\n");
		$userBuiltFlow->{_flow_color}  			 = $flow_color;
		
	} else  {
	 	print("blue_flow, set_flow_color, missing color\n");
	}
	return();
 }
...
sub _stack_versions {
	my $flow_listbox_color_w 			= _get_flow_listbox_color_w();
	   			print("blue_flow, _stack_versions ,userBuiltFlow->listbox_widget: $flow_listbox_color_w\n");
...
#   	my ($self) = @_;
#   	
#   	$userBuiltFlow->{_flow_type} = $flow_type->{_user_built}; # should be at start of blue_flow
#   	 		 print("blue_flow, sunix_select,parameter_values_frame: $parameter_values_frame\n"); 	
#   	  	
#   	use messages::message_director; 
#   	use param_sunix;
#   	  	
...
#	_check4changes();
#	
#     # print("blue_flow,1. sunix_select,flow_color: $userBuiltFlow->{_flow_color}\n"); 	
#   	  	     
#    $conditions_gui						    ->set_gui_widgets($userBuiltFlow);
#    $conditions_gui							->set4start_of_sunix_select();
#    #$userBuiltFlow->{_flow_color} 			= $conditions_gui->get_flow_color();
#    my $flow_color							= $userBuiltFlow->{_flow_color};
#    	print("blue_flow,2. sunix_select,flow_color: $flow_color\n"); 
#    	# print("1. blue_flow, sunix_select, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");
...
#				print("blue_flow sunix_select, program name is ${$userBuiltFlow->{_prog_name_sref}}\n");
#   	$param_sunix   							->set_program_name($userBuiltFlow->{_prog_name_sref});
#   	$userBuiltFlow->{_names_aref}  					= $param_sunix->get_names();
#   	$userBuiltFlow->{_values_aref} 					= $param_sunix->get_values();
#   	$userBuiltFlow->{_check_buttons_settings_aref}  = $param_sunix->get_check_buttons_settings();
...
#
#   	$param_widgets      ->set_location_in_gui($here);
#   				# print("2. blue_flow, sunix_select, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");
#   	   			# widgets initialized in super class	
#	#$param_widgets		->set_labels_w_aref($userBuiltFlow->{_labels_w_aref} );
#  	#$param_widgets		->set_values_w_aref($userBuiltFlow->{_values_w_aref} );
#  	#$param_widgets		->set_check_buttons_w_aref($userBuiltFlow->{_check_buttons_w_aref});
...
#						$userBuiltFlow->{_check_buttons_settings_aref});
#   	$param_widgets		->set_current_program($userBuiltFlow->{_prog_name_sref});
#		# print("3. blue_flow, sunix_select, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");
#						 # print("blue_flow sunix_select, $userBuiltFlow->{_is_sunix_listbox}\n");
#   	$param_widgets       ->set_location_in_gui($here);
#   	$param_widgets		->redisplay_labels();
#   	$param_widgets		->redisplay_values();
#   	$param_widgets		->redisplay_check_buttons();
#
#    $conditions_gui		->set4end_of_sunix_select() ;
#     # print("blue_flow,2. sunix_select,1 line after set4end_of_sunix_select\n");
#    # $flow_color				= $userBuiltFlow->{_flow_color};
#    # TODo are  following 1 line and past 1 line needed?
#    # $userBuiltFlow 		= $conditions_gui->get_hash_ref();
#    	# print("4. blue_flow, sunix_select, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");
#	 # print("blue_flow,3. sunix_select,flow_color: $flow_color\n");
#   	return();
#}
...
sub _check4changes {
	my $self = @_;	
		...
		# print("blue_flow check4changes: _last_flow_index_touched $userBuiltFlow->{_last_flow_index_touched}\n");
	...
	if ($userBuiltFlow->{_last_flow_index_touched} >= 0) {  # <-1 does exist and means the flow was not touched
		...
		my $last_idx_chng =$userBuiltFlow->{_last_flow_index_touched} ;
	 					      ...
	 					      #  print("blue_flow_check4changes,
	 					      # last changed entry index was $last_idx_chng\n");
  							  # print("blue_flow _check4changes program name is 
  							  # ${$userBuiltFlow->{_prog_name_sref}}\n");
		...
		$userBuiltFlow->{_check_buttons_settings_aref}	= $param_widgets	->get_check_buttons_aref();
  							...
  							#print("blue_flowflow_select,changed values_aref: @{$userBuiltFlow->{_values_aref}}\n");
  							#print("blue_flow_changes4changes,changed names_aref: @{$userBuiltFlow->{_names_aref}}\n");
  							#print(" blue_flowflow_select,changes, check_buttons_settings_aref: @{$userBuiltFlow->{_check_buttons_settings_aref}}\n");
  							#
   		 $param_flow	-> set_flow_index($last_idx_chng);
  						 	     print("blue_flow check4change ,store changes in param_flow, last changed entry index $last_idx_chng\n");
		 ...
		 $param_widgets	->set_entry_change_status($false);  # changes are now complete
	 	 						 # print("blue_flow flow_select, set_entry_change_status: to 0\n");
	}  						
...
=head2 sub flow_select
  ...
  if($userBuiltFlow->{_prog_name_sref}) {
 		print("\n1. blue_flow flow_select, program name is $userBuiltFlow->{_prog_name_sref}\n");
 	}
...
sub flow_select {
    ...
    $userBuiltFlow->{_flow_type} = $flow_type->{_user_built}; 
			print("1. blue_flow, flow_select, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");
	my $message          		= $userBuiltFlow_messages->null_button(0);
 	$message_w   				->delete("1.0",'end');
 	$message_w   				->insert('end', $message);
 				print("blue_flow, flow_select,flow color: $flow_color \n");
  	$conditions_gui				-> set_gui_widgets($userBuiltFlow);
 	$conditions_gui				-> set_hash_ref($userBuiltFlow);
	...
	my $flow_listbox_color_w 	= _get_flow_listbox_color_w();
 	...
 	my $flow_color				= $userBuiltFlow ->{_flow_color};  # TODO: careful, color is re-assigned from conditions_gui,few lines up
 				# print("blue_flow, flow_select,_last_flow_listbox_touched_w: $userBuiltFlow->{_last_flow_listbox_touched_w} \n");
	...
	$decisions		 		 				->set4flow_select($userBuiltFlow);
			# print("2. blue_flow, flow_select, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");	
	my $pre_req_ok 	  						= $decisions->get4flow_select();
				# print ("blue_flow, flow_select, pre_req_ok: $pre_req_ok\n");
	...
	if ($pre_req_ok) {
 		...
 		$userBuiltFlow->{_prog_name_sref} 	= $flow_widgets->get_current_program(\$flow_listbox_color_w);
  		...
  		if( $flow_widgets->is_drag_deleted($flow_listbox_color_w) ) {
			...
			$flow_widgets->set_vigil_on_delete();
				...
				# print("3. blue_flow, flow_select, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");
		                     # find out which program was previously touched
		                     # assume all prior programs touched have
		                     # modified parameters 
		                     # and update that program's stored values	
		_check4changes();
				# print("4 blue_flow, flow_select, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");                     
		                    # for just-selected program name
							# get its flow parameters from storage
							# and redisplay the widgets with parameters 
   		my $index 							= $flow_widgets->get_flow_selection($flow_listbox_color_w);
 				# print("blue_flow, flow_select,_flow_listbox_color_w: $userBuiltFlow->{_flow_listbox_color_w} \n");
  							# print("2. blue_flow, flow_select,index is $index\n");
  							my $num_items = $param_flow->get_num_items();
 		$param_flow							->set_flow_index($index);
 		$userBuiltFlow->{_names_aref} 		= $param_flow->get_names_aref();

  							# print("2. blue_flow flow_select,parameter names: @{$userBuiltFlow->{_names_aref}}\n");
 		$userBuiltFlow->{_values_aref} 		=	$param_flow->get_values_aref();

  				 			 print("3. blue_flow flow_select,parameter values:@{$userBuiltFlow->{_values_aref}}\n");
 		$userBuiltFlow->{_check_buttons_settings_aref} 
									=  $param_flow->get_check_buttons_settings();

  				 			 # print("4. blue_flow flow_select, check_buttons_settings no changes, @{$userBuiltFlow->{_check_buttons_settings_aref}}\n");
  				 			# print("4. blue_flow flow_select,index;$index\n#");
  				 			
 				 			# get stored first index and num of items 
 		$userBuiltFlow->{_param_flow_first_idx}	= $param_flow->first_idx();
 		$userBuiltFlow->{_param_flow_length}   	= $param_flow->length();
 		$param_widgets				 			->set_current_program($userBuiltFlow->{_prog_name_sref});
 				 print("5. blue_flow, flow_select, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");

				 			  #print("blue_flow, flow_select, $userBuiltFlow->{_is_flow_listbox_color_w}\n");
				 			  # print("blue_flow, flow_select, color:$flow_color\n");
		$whereami				->set4flow_listbox($flow_color);
		$here 					= $whereami->get4flow_listbox();
				 			  # print("5. blue_flow flow_select, $here->{_is_flow_listbox_color_w}\n");
				 			  # print("blue_flow, flow_select, _is_flow_listbox_color_w, $here->{_is_flow_listbox_color_w}\n");

		# $param_widgets		->range($userBuiltFlow); # to set widgets, must be before gui_clean
		 				 print("6. blue_flow, flow_select, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");	
		...
		$param_widgets		->set_labels_w_aref($userBuiltFlow->{_labels_w_aref} );
 		...
 		$param_widgets				->set_check_buttons(
										$userBuiltFlow->{_check_buttons_settings_aref});
								# TODO
					  		 # print("6. blue_flow flow_select, is listbox selected:$here->{_is_flow_listbox_color_w}\n");
		$whereami					->set4flow_listbox($flow_color);
		$here 						= $whereami->get4flow_listbox();
				# print("6. blue_flow, flow_select, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");
    	$param_widgets      		->set_location_in_gui($here);
		...
		my @Entry_widget			= @{$param_widgets->get_values_w_aref()};
					# print("blue_flowflow_select,Entry_widgets@Entry_widget\n");
		$Entry_widget[0]->focus;  # always put focus on first entry widget
 		...
 		$binding					->set();
				# print("7. blue_flow, flow_select, _values_w_aref $userBuiltFlow->{_values_w_aref}\n");
		
    	$conditions_gui				->set4end_of_flow_select($flow_color);
    	$conditions_gui				->set_flow_index_last_touched($index);
    	
    	$userBuiltFlow 				= $conditions_gui->get_hash_ref(); # now blue_flow = 0; flow_type=user_built
    		# print("6. blue_flow,flow_select,flow_color: $userBuiltFlow->{_flow_color} \n");
   		return();
	}
}
