...
=head2 
  ...
  121 of convenient private abbreviated-variable names
...
sub _FileDialog {
    ...
    my $file_path	   =  _get_file_path();    # e.g., $PL or $DATA_SEISMIC_SU
 ...
 sub _get_file_path {
	...
	if ($file_dialog->{_path}) {
		...
		return($path);
	 	...
	 	print(" file_dialog,_get_file_path, missing fiel path\n");
	return();		
	}		
 }
...
=cut
 ...
 sub _pre_built_superflow_close_data_file {
	...
	if ($full_path_name ) {
		...
		print("_pre_built_superflow_close_data_file,Cancelled. No output flow name selected\n");
	}	
	...
	my $pre_req_ok 		= $decisions->get4FileDialog_select();
   	...
   	if ( $pre_req_ok ) { 	
		...
		$conditions_gui				-> set4superflow_close_data_file_end();
   		$file_dialog				 = $conditions_gui->get_hash_ref();  # retrieves 91
...
=cut 
 ...
 sub _pre_built_superflow_open_data_file {
	...
	$file_dialog				 = $conditions_gui->get_hash_ref(); # gets 91	
...
#		_user_built_flow_save_perl_file();
 ...
 sub  _set_FileDialog2pre_built_superflow {	
	...
	if ($topic eq $file_dialog_type->{_Data}     ) {		
		...
		_pre_built_superflow_open_data_file(); # only dialog possible in subroutine
...
=cut
 ...
 sub _user_built_flow_SaveAs_perl_file {
	...
	if ( $pre_req_ok ) {
		...
		# print("3. file_dialog, _user_built_flow_SaveAs_perl_file ,SaveAs, pre_req_ok= $pre_req_ok \n");
		...
		# print ("file_dialog _user_built_flow_SaveAs_perl_file, last_parameter_index_touched: $file_dialog->{_last_parameter_index_touched} \n");
		...
		$full_path_name 			= $file_dialog->{_selected_file_name};
			# print("file-dialog _user_built_flow_SaveAs_perl_file, full_path_name: $full_path_name\n");

		if ($full_path_name ) {
  			@fields 								= split (/\//,$full_path_name);
			$file_dialog->{_is_selected_file_name}	= $true;
		} else {
			print("file_dialog, _user_built_flow_SaveAs_perl_file,Cancelled. No output flow name selected\n");
		}
		...
		# print ("file-dialog _user_built_flow_SaveAs_perl_file, last_parameter_index_touched: $file_dialog->{_last_parameter_index_touched} \n");
...
sub _user_built_flow_close_data_file  {
	...
	if ($full_path_name ) {		
		...
		print("file_dialog,_user_built_flow_close_data_file, Cancelled. No output flow name selected\n");
	}	
	...
	my $pre_req_ok 		= $decisions->get4FileDialog_select();
   	...
   	if ( $pre_req_ok ) { 	
     	...
     	$file_dialog->{_last_parameter_index_touched}  = $current_index;	
		...
		if ( $file_dialog->{_is_flow_listbox_grey_w}  	|| 
			...
			@{$file_dialog->{_values_aref}}[$current_index] = $file_dialog->{_selected_file_name};  
			 		# print("file_dialog, _user_built_flow_close_data_file, selected_file_name: $file_dialog->{_selected_file_name} \n");
			$param_widgets->set_values($file_dialog->{_values_aref});
		    $param_widgets->redisplay_values();
...
sub _user_built_flow_close_perl_file  {
	...
	if ($full_path_name ) {
		...
		print("_user_built_flow_close_perl_file,Cancelled. No output flow name selected\n");
	}	
		...
		if ( $pre_req_ok ) {
			...
			$file_dialog->{_path} 				= $iFile->get_Open_perl_flow_path();
				print("file_dialog,_user_built_flow_close_perl_file,file name = $fields[-1]\n");
				print("file_dialog,_user_built_flow_close_perl_file,path = $file_dialog->{_path}\n");		 
		...
		#  				my $suffix 							= $control->get_suffix();
		#  				my $first_name						= $control->get_first_name();
  							   	# print("file_dialog,_user_built_flow_close_perl_file, open,suffix: $suffix\n");
#
#   				#$file_dialog->set4FileDialog_open_start(); #needed?
 ...
 sub _user_built_flow_open_data_file {
	...
	my $iFile					= iFile				->new();
 	...
 	my $length  			 	= $default_param_specs->{_length};
  				...
  				# print ("1. file_dialog,_user_built_flow_open_data_file, is_last_parameter_index_touched: $file_dialog->{_is_last_parameter_index_touched} \n");
				# print ("1. file_dialog,_user_built_flow_open_data_file, is_last_flow_index_touched: $file_dialog->{_is_last_flow_index_touched} \n");
           		...
           		# print (" 2. file_dialog,_user_built_flow_open_data_file->{_last_parameter_index_touched}: $file_dialog->{_last_parameter_index_touched}\n");
	$conditions_gui				->set_hash_ref($file_dialog);
	$conditions_gui				->set_gui_widgets($file_dialog);
	$conditions_gui   			->set4FileDialog_open_start();
	...
	if ($widget_type eq 'Entry' ) {
		...
		if ($file_dialog->{_parameter_value_index} >= 0) { # for additional certainty; but is it needed?
			...
			$iFile 						-> set_prog_name_sref($file_dialog);
			$file_dialog->{_path} 		= $iFile->get_Data_path();	
  				# print("6. file_dialog,_user_built_flow_open_data_file $file_dialog->{_path}\n");						
			# collects the name of the data file to be opened
  			_FileDialog();
 ...
 sub _user_built_flow_open_perl_file {
	...
	my $open_perl_file_messages = message_director->new();
 	...
 	my $length  			 	= $default_param_specs->{_length};
           		...
           		# print (" 2. file_dialog,_user_built_flow_open_data_file->{_last_parameter_index_touched}: $file_dialog->{_last_parameter_index_touched}\n");
	$conditions_gui				->set_hash_ref($file_dialog);
	$conditions_gui				->set_gui_widgets($conditions_gui);
	$conditions_gui   			->set4FileDialog_open_start();
	...
	# my $widget_type		= $whereami->widget_type($parameter_values_frame);
	 			# print("file_dialog  _user_built_flow_open_perl_file selected widget type is = $widget_type	\n");


#	if ($widget_type eq 'Entry' ) { # travel from user-built-flow
#				     
#				   		# print("1. file_dialog,_user_built_flow_open_data_file, selected widget_type=$widget_type \n");
#
#   		my $selected_Entry_widget 	= $parameter_values_frame->focusCurrent;
...
#  				# print("6. file_dialog,_user_built_flow_open_data_file End\n");
#		} 
#	} elsif  ($widget_type eq 'MainWindow') {  # opening the first file before gui is populated
					# print("file_dialog,_user_built_flow_open_perl_file widget type is 'MainWindow' \n");
 		...
 		$file_dialog->{_path}  			= $iFile->get_Open_perl_flow_path();	
  		...
  		_user_built_flow_close_perl_file();
 	  	
#	} else {
		
#		print("file_dialog,_user_built_flow_open_perl_file no widget type selected \n");
#	}
 ...
 sub get_hash_ref {
    ...
    if($file_dialog) {
    	...
    	$file_dialog->{_is_flow_item_down_arrow_button}	= $is_flow_item_down_arrow_button;
 		...
 		$file_dialog->{_is_last_parameter_index_touched} 	= $is_last_parameter_index_touched;	
    	...
    	$file_dialog->{_last_parameter_index_touched_green}	= $last_parameter_index_touched_green;
     	$file_dialog->{_last_parameter_index_touched_blue}	= $last_parameter_index_touched_blue;
      	$file_dialog->{_last_parameter_index_touched}		= $last_parameter_index_touched;
...
=head2 sub get_last_parameter_index_touched


=cut 

 sub get_last_parameter_index_touched {
	my ($self)   = @_;
	
	if ($file_dialog->{_last_parameter_index_touched} >= 0) {	
		
		my $change 					= $file_dialog->{_last_parameter_index_touched};
		# print("file_dialog,get_last_parameter_index_touched, _last_parameter_index_touched: $file_dialog->{_last_parameter_index_touched}\n");
		return($change);
		
	} else  { 
		# print("file_dialog,get_last_parameter_index_touched, _last_parameter_index_touched = -1 \n");
		my $change 					= $file_dialog->{_last_parameter_index_touched};
		return($change);
	}
 }
 ...
 sub set_hash_ref {
 	...
 	if ( $hash_ref) {
    	...
    	$file_dialog->{_is_Save_button} 					= $hash_ref->{_is_Save_button};	
 		...
 		$file_dialog->{_is_last_parameter_index_touched}	= $hash_ref->{_is_last_parameter_index_touched};
    	...
    	$file_dialog->{_last_flow_index_touched_grey}		= $hash_ref->{_last_flow_index_touched_grey};
     	$file_dialog->{_last_flow_index_touched_pink}		= $hash_ref->{_last_flow_index_touched_pink};
      	...
      	$file_dialog->{_last_flow_index_touched_blue}		= $hash_ref->{_last_flow_index_touched_blue};
       	$file_dialog->{_last_parameter_index_touched}		= $hash_ref->{_last_parameter_index_touched};
    	...
    	$is_last_parameter_index_touched		= $hash_ref->{_is_last_parameter_index_touched};
...
=cut
 	...
 	if ( $widget_hash_ref) {
		...
		$file_dialog->{_add2flow_button_grey}		= $widget_hash_ref->{_add2flow_button_grey};
 		...
 		$file_dialog->{_flow_name_pink_w} 			= $widget_hash_ref->{_flow_name_pink_w};	
  		$file_dialog->{_flow_name_green_w} 			= $widget_hash_ref->{flow_name_green_w}; 		
 ...
 sub FileDialog_director {
	...
	# print ("1. file_dialog,FileDialog_director, is_last_parameter_index_touched: $file_dialog->{_is_last_parameter_index_touched} \n");
