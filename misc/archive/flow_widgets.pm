...
sub drag_end {
   ...
   if($nearest < 0) {
		...
		my @xy = $site->bbox($nearest); 
        $xmax_destination_box  = $xy[0] + $xy[2]; 
        $xmin_destination_box  = $xy[0]; 

        $ymax_destination_box  = $xy[1] + $xy[3]; 
        $ymin_destination_box  = $xy[1]; 
        $min_y_listbox_inuse   = $xy[0];   # $xy[0] = 3
        $max_y_listbox_inuse   = $last_idx * ($xy[3] + $min_y_listbox_inuse) + $min_y_listbox_inuse  ;

						print(" 3 flow_widgets, drag_end, destination item's x,y dx, dy in lb coords\n");
        				# print(" 4 Destination box xy values in lb coords@xy  \n");
        				...
        				print(" 7 YMin YMax box area of lb in use: $min_y_listbox_inuse, $max_y_listbox_inuse \n");
  						...
  						# ARROW Lies within Top_Bott boundaries of listbox  ?
        if (($min_y_listbox_inuse  <= $yarrow)  && 
             $max_y_listbox_inuse  >= $yarrow) {  
            ...
            if($nearest == $last_idx) {  # CASE 3 
               ...
               $flow_widgets->{_chosen_index2drop} =  $final_insertion_point ;
  					...
  					# CASE 4 -5 Lies outside Top or Bottom boundaries of listbox  ?
            					...
            					# print("xarrow yarrow $xarrow $yarrow");
            					# CASE4 neareset is still >= 0
            					# print("outside Top-Bottom boundaries $min_y_listbox_inuse $max_y_listbox_inuse \n");
           if ($max_y_listbox_inuse  <= $yarrow) { #CASE4
	         	$final_insertion_point 				= $last_idx;
             	$site->insert( 'end', $text );
           	    $flow_widgets->{_chosen_index2drop} =  $final_insertion_point ;
            				 print("13.flow_widgets  Drop, insertion index is $flow_widgets->{_chosen_index2drop}\n");
            
		   } 				 # CASE 5 is handled by drag and drop whivh deletes the dragged item automatically
           if ($min_y_listbox_inuse  >= $yarrow) {
              $site->insert( 0, $text );
             	$flow_widgets->{_chosen_index2drop} =  0 ;
            				 print("14. Drop, insertion index is 0\n");
		   }
		 }	# CASE4-5 nearest is still >= 0
   } # nearest is still >= 0
...
=cut
 ...
 if(@selection_index) {
     ...
     # print("flow_widgets  get_current_program program name is $prog_name\n");
 	return(\$prog_name);
	 }else{
 		# print("flow_widgets,get_current_program, Error: null selection index\n");
 		return();
 	}
