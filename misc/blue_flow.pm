...
=head3 NOTES
  ...
  if widget_name= menubutton we have superflow 
   ...
   2. Set the new program name 
    ...
    flow_listbox_blue_w  	-bottom right listbox,input by user selection
...
=cut 
	...
	use conditions_gui;
 	...
 	# print("color flow, make param_flow instance blue: $param_flow_blue\n");
  		...
  		if ($pre_req_ok) {
			...
			if ($index == 0 && $param_flow_color_i->get_num_items == 1) {
				...
				# the last program that was touched is cancelled out
   				...
   				$color_flow->{_is_last_flow_index_touched_blue} 	= $false;
...
sub flow_select {
	my ($self) = @_;
	# print("2. color_flow, flow_select,flow color: $color_flow->{_flow_color} and flow_color $flow_color \n");
	use messages::message_director;
	use decisions;
	...
	#TODO: careful, color is re-assigned from conditions_gui,few lines up ... necessary?
 	my $flow_color				= $color_flow ->{_flow_color}; 
 				# print("11. blue_flow, flow_select,flow color: $color_flow->{_flow_color} = flow_color $flow_color \n");
	...
	if ($pre_req_ok) {
    	...
    	$prog_name_sref 	= $color_flow->{_prog_name_sref};
   		...
   		return();
  			...
  			# print("22. blue_flow, flow_select,flow color: $color_flow->{_flow_color} = flow_color $flow_color \n");
}
