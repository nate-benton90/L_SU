...
=cut
 ...
 sub _FileDialog_button {   # N.B. Another FileDialog_button in L_SU needs to be updated too!!!!!!!
	...
	if ($option_sref)  {
		...
		$userBuiltFlow->{_last_parameter_index_touched_color} = $file_dialog->get_last_parameter_index_touched_color();
...
=head2  _check4flow_changes
	...
	if ($param_widgets->get_entry_change_status && $userBuiltFlow->{_last_flow_index_touched} >= 0) {		
		 ...
		 ( $userBuiltFlow->{_last_parameter_index_touched_color} ) >= 0)        
=cut 
...
sub _check4flow_changes {
   ...
   # print("neutral_flow check4flow_changes: _last_parameter_index_touched_color $userBuiltFlow->{_last_parameter_index_touched_color}\n");
...
sub _check4parameter_changes {
	my $self = @_;	
   		...
   		# print("B4 neutral_flow check4changes: _last_parameter_index_touched_color: $userBuiltFlow->{_last_parameter_index_touched_color}\n");
   
	if ( $userBuiltFlow->{_last_parameter_index_touched_color} >= 0 ){  # <-1 does exist and means the parameters are untouched

		my $last_parameter_idx_touched = $userBuiltFlow->{_last_parameter_index_touched_color} ;
		...
		$param_flow						-> set_flow_index($current_flow_listbox_index);
		 ...
		 $param_widgets	->set_entry_change_status($false);  # changes are now complete
	 	 ...
	 	 $userBuiltFlow->{_last_parameter_index_touched_color} = -1;
	 	...
	 	# print("End neutral_flow check4changes: _last_flow_index_touched $userBuiltFlow->{_last_flow_index_touched}\n");
   		# print("BEndneutral_flow check4changes: _last_parameter_index_touched_color reset: $userBuiltFlow->{_last_parameter_index_touched_color}\n");
	}  						
 ...
 sub FileDialog_button {   # N.B. Another _FileDialog_button in L_SU needs to be updated too!!!!!!!
	...
	if ($option_sref)  {
		...
		if ($topic eq $file_dialog_type->{_SaveAs} ) {
					...
					# print ("neutral_flow,FileDialog_button, last_parameter_index_touched_color: $userBuiltFlow->{_last_parameter_index_touched_color} \n");
...
=cut
 ...
 sub get_last_flow_color {
	...
	if ($userBuiltFlow ->{_flow_color}) {
		...
		$userBuiltFlow ->{_last_flow_color} = $userBuiltFlow ->{_flow_color};
		
		# for export
		$last_flow_color		= $userBuiltFlow ->{_last_flow_color};
		...
		$last_flow_color		= $hash_ref->{_last_flow_color}; # used in flow_select  	 	
 		...
 		$userBuiltFlow->{_flow_item_up_arrow_button}	= $hash_ref->{_flow_item_up_arrow_button};		
 	 	...
 	 	$userBuiltFlow->{_flow_name_green_w}			= $hash_ref->{_flow_name_green_w};
   		...
   		$userBuiltFlow->{_last_flow_color}				= $hash_ref->{_last_flow_color};   		
...
sub sunix_select {
	...
	_check4flow_changes();
     	...
     	# print("neutral_flow,1. sunix_select,_is_flow_listbox_grey_w:	$userBuiltFlow->{_is_flow_listbox_grey_w} \n");	
    ...
    $conditions_gui		->set4end_of_sunix_select();
    $userBuiltFlow->{_last_flow_color} = $flow_color;
