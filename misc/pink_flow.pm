...
=cut 
	...
	use conditions_gui;
 	...
 	# print("color flow, make param_flow instance pink: $param_flow_pink\n");
...
sub _set_is_last_parameter_index_touched_color {	
	...
	if ($flow_color eq 'pink') {
		...
		# not for export
		$color_flow->{_is_last_parameter_index_touched_pink}	=  $color_flow->{_is_last_parameter_index_touched_pink};
  		...
  		if ($pre_req_ok) {
			...
			if ($index == 0 && $param_flow_color_i->get_num_items == 1) {
				...
				# the last program that was touched is cancelled out
   				...
   				$color_flow->{_is_last_flow_index_touched_pink} 	= $false;
...
sub flow_select {
	...
	#TODO: careful, color is re-assigned from conditions_gui,few lines up ... necessary?
 	my $flow_color				= $color_flow ->{_flow_color}; 
 				# print("11. pink_flow, flow_select,flow color: $color_flow->{_flow_color} = flow_color $flow_color \n");
 		...
 		$color_flow->{_names_aref} 			= $param_flow_color_i->get_names_aref();
  		$color_flow->{_values_aref} 		= $param_flow_color_i->get_values_aref();
  							...
  							# print("2. color_flow flow_select,param_flow_pink: $param_flow_pink\, flow_color: $flow_color\n");  						
  				 			  ...
  				 			  # print("3. color_flow flow_select,parameter values:@{$color_flow->{_values_aref}}[1]\n");
...
# 
   		...
   		return();
  			...
  			# print("22. pink_flow, flow_select,flow color: $color_flow->{_flow_color} = flow_color $flow_color \n");
}
