 ...
 sub _get_names_aref {
 	my ($item_index) = @_;
  	...
  	if ($item_index >=0 ) {
		...
		# print("param_flow, _get_names_aref,
 ...
 sub _get_values_aref {
 	my ($item_index) = @_;
  	...
  	if ($item_index >=0 ) {
		...
		#print("param_flow, _get_values_aref, $values_aref[0], index=$item_index\n");
  		return (\@values_aref);
	} 
 } 
...
sub	_set_good_labels4item {
	...
	my (@good);
	my ($j) ;
    					# print("1. param_flow,_set_good_labels4item, 
	...
	for ( my $i=0, $j=0; $i < $length; $i++ ) {
   		 				# print("param_flow, _set_good_labels4item:
   		 				# values_aref is @$values_aref[$i]\n");
   		   		 
   		if ( @$values_aref[$i] ) {  # if values are initialized
	  		...
	  		if (@$values_aref[$i] ne "'nu'" && 
	  		 @$values_aref[$i] ne '') {
				...
				my $name = ${@{$param_flow->{_names_aref2}}[$idx]}[$i];
    					# print("2. param_flow,_set_good_labels4item,
    					# good index #$i, name:$name \n");  					
				$good[$j]	= $name;
				$j++;
	  			}
	  	}  else {
	  		# print("param_flow, _set_good_labels_4item: no values are present, can not be saved\n"); 		
	  	}	 								
	} 
	...
	#print("	3. param_flow,_set_good_labels4item,
	#num_good_labels= $num_good_labels[$idx] , 
	#labels/names are:	@{@{$param_flow->{_good_labels_aref2}}[$idx]}\n");
...
sub	_set_good_values4item {
	my($index4flow) = @_;
	 	# print("param_flow,set_good_indices4item,index4flow: $index4flow \n");
	 					
	my $idx = $index4flow;	# program sequence in flow
	my (@good);
	my ($j) ;

    	# print("1. param_flow,_set_good_values4item, flow index:$idx, prog name:@{$param_flow->{_prog_names_aref}}[$idx] \n");
    					
    my $values_aref  	= _get_values_aref($idx);
	my $length 			= scalar @$values_aref;
		# print("2. param_flow,_set_good_values4item, length: $length\n");

	for ( my $i=0, $j=0; $i < $length; $i++ ) {
   		 	#print("param_flow, _set_good_values_4item: values_aref is @$values_aref[$i]\n");
	  	...
	  	if ( @$values_aref[$i] ) {
	  		...
	  		if (@$values_aref[$i] ne "'nu'" && 
	  		 @$values_aref[$i] ne '') {
    			...
    			# print("2. param_flow,_set_good_values4item,good index #$i\n");
    			# print("2. param_flow,_set_good_values4item,value:$value \n");		
				$good[$j]	= $value;
				$j++;
	  		}
	  	}  else {
	  		# print("param_flow, _set_good_values_4item: no values are present, can not be saved\n"); 		
	  	}	
	} 

	$num_good_values[$idx] 				= $j;
			# print("param_flow,_set_good_values4item,good_values=@good \n");
	...
	# print("	3. param_flow,_set_good_values4item,num_good_values= $num_good_values[$idx], values are:	@{@{$param_flow->{_good_values_aref2}}[$idx]}\n");
	return();
}
...
=cut
 ...
 sub clear {
 	...
 	$param_flow->{_checkbuttons_aref}		= '';
	$param_flow->{_checkbuttons_aref2}		= '';
	$param_flow->{_destination_index}		= '';
	$param_flow->{_end}						= '';
	$param_flow->{_first_idx}				= 0;
	$param_flow->{_good_checkbuttons_aref2}	= '';
	$param_flow->{_good_labels_aref2}		= '';
	$param_flow->{_good_values_aref2}		= '';
	$param_flow->{_index2move}				= '';
 	$param_flow->{_index4flow}				= -1;
 	$param_flow->{_index4checkbuttons}		= -1;
 	$param_flow->{_index4names}				= -1;
 	$param_flow->{_index4values}			= -1;
	$param_flow->{_indices}					= -1;
	$param_flow->{_label_boxes_w}			= '';
	$param_flow->{_length}					= '';
	$param_flow->{_names_aref}				= '';
	$param_flow->{_names_aref2}				= '';
	$param_flow->{_num_good_values_aref}	= 0;
	$param_flow->{_num_good_labels_aref}	= 0;
	$param_flow->{_num_items}				= 0;
 	$param_flow->{_num_items4flow} 			= 0;
 	$param_flow->{_num_items4checkbuttons} 	= 0;
 	$param_flow->{_num_items4names} 		= 0;
 	$param_flow->{_num_items4values}		= 0;
	$param_flow->{_prog_names_aref}			= '';
	$param_flow->{_prog_version_aref}		= 0;
	$param_flow->{_selection_index}		    = '';
	$param_flow->{_start}					= 0;
	$param_flow->{_values_aref}				= '';
	$param_flow->{_values_aref2}			= '';
...
=cut
 ...
 sub get_names_aref {
	my ($self) 	= @_ ;
	my $index 	= $param_flow->{_selection_index};
		# print(" param_flow, get_name_aref names: index is $index\n");
	...
	if ($index >=0) {
		...
		# print(" param_flow, get_names_aref:  @names_aref, index is $index\n");
		return (\@names_aref);
  } else {
  	print(" param_flow, get_names names:index <0 \n");
  }
 }
...
=cut
 ...
 sub get_num_items {
   ...
   if ( $param_flow->{_num_items} >= 0 ) {
   	my $num_items =  $param_flow->{_num_items4flow};
   	
   	   # print("param_flow,get_num_items, num_items = $param_flow->{_num_items} \n");
   	   return($num_items);
   	   
   }else {
   	print("param_flow,get_num_items,- no number of items \n");
   	return();
   }
...
=cut
 ...
 sub get_values_aref {
 	my ($self) 	= @_ ;
 		...
 		# print("param_flow, get_values :_selection_index $param_flow->{_selection_index}\n");
 		   
  	if ( ($param_flow->{_selection_index} >=0) && 
  	    $param_flow->{_values_aref2}) {
  	    	
  		my $index = $param_flow->{_selection_index};
  		# print("param_flow, get_values :_values_aref2:@{@{$param_flow->{_values_aref2}}[$index]}\n");	
  		my ($i,$j,$length);
  		my (@values_aref,@values);
  			# print("param_flow, get_values :index $index\n");
		...
		print ("param_flow,get_values_aref :selection_index <=0  or values_aref2\n");
		return();
	} 
 } 
...
sub	set_good_values {
	...
	my($self) = @_;
	my $length = $param_flow->{_num_items4flow}; # no. programs in flow

	  # print("param_flow,set_good_values num_items4flow:	$param_flow->{_num_items4flow}\n");
...
=cut
 ...
 sub set_values_aref {
 	...
 	if ($param_flow->{_selection_index} >= 0) {
 		...
 		print("param_flow, set_values_aref: missing selection index\n");		
 	}
    		# print("param_flow,set_values_aref :values_aref are @$values_aref[0]\n");
			# print("param_flow,set_values_aref, index: $index\n");					
 return ();
 } 
...
=head2 sub stack_names_aref2 
 ...
 # print("param_flow, stack_names_aref2, names in program# $index are @{$names[$index]}[0]\n");
 for(my $i = 0; $i <=$index; $i++) {
   	print("param_flow,stack_names_aref2, cumulative first names are:  @{$names[$i]}[0], prog num =$i\n");
 }
 ...
 sub stack_names_aref2 {
  ...
  if ($names_aref) {
  	my $index    						= $param_flow->{_index4names}  + 1;
  		# print("param_flow,names_aref2,index: $index\n");
...
#     
#  	for (my $i=0; $i<=$index;$i++) {
#     	print("param_flow_grey,stack_names_aref2, an accumulating array of arrays: @{$param_flow->{_names_aref2}} item $i\n");
#  	} 
...
#  		for (my $i=0; $i<=$index;$i++) {
#     		print("param_flow_grey,stack_values_aref2, an accumulating array of arrays: @{$param_flow->{_values_aref2}} item $i\n");
#  		} 
