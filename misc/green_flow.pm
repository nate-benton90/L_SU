...
=cut 
	...
	use conditions_gui;
 	...
 	# print("color flow, make param_flow instance green: $param_flow_green\n");
...
=head2  _check4flow_changes
	...
	Assume now that selection of a flow item implies that pre-existing paramters were changed
	Assume that opening a file dialog implies paramter values were changed (Entry widget) 
	-- this means that a prior program must have been touched
	...
	The only exception is when you have just left a flow of a different color and are returning
...
sub _check4flow_changes {
	...
	if ( ( $color_flow->{_last_flow_index_touched} >= 0  ||
		 $color_flow->{_last_parameter_index_touched_color} >= 0 )  &&
		 ($color_flow->{_last_flow_color} eq '' || # empty case
		...
		my $last_idx_chng = $color_flow->{_last_flow_index_touched} ;
	 				...
	 				last changed flow index was $last_idx_chng\n");
		...
		$color_flow->{_check_buttons_settings_aref}	= $param_widgets	->get_check_buttons_aref();
  							...
  							# print("31 grey_flow check4flow_changes,changed values_aref: @{$color_flow->{_values_aref}}[0..1]\n");
		...
		# save old changed values and names
		...
		# print("color_flow flow_select, set_entry_change_status: to 0\n");
...
=head2  _check4parameter_changes
	...
	Assume that the program of interest within an active flow does not change
...
sub _check4parameter_changes {
	...
	if ( $color_flow->{_last_parameter_index_touched_color} >= 0 ){  # N.B., <-1 does exist and means the parameters are untouched
		...
		print(" color_flow,_check4parameter_changes paramters are untouched \n");
	}						
...
# 			print("color_flow,FileDialog_button,_is_flow_listbox_blue_w:	$color_flow->{_is_flow_listbox_blue_w} \n");
			...
			$file_dialog		->set_flow_type('user_built');
					# print("3  color_flow,FileDialog_button,_flowNsuperflow_name_w:$color_flow->{_flowNsuperflow_name_w} \n");
#				print("color_flow,1 FileDialog_button, flow_name_color_w,   $color_flow->{_flow_name_green_w} \n");
			$file_dialog		->FileDialog_director();
					# print("3  color_flow,FileDialog_button,_flowNsuperflow_name_w:$color_flow->{_flowNsuperflow_name_w} \n");
#					print("color_flow,2 FileDialog_button, flow_name_color_w,   $color_flow->{_flow_name_green_w} \n");			
  		...
  		if ($pre_req_ok) {
			...
			if ($index == 0 && $param_flow_color_i->get_num_items == 1) {
				...
				# the last program that was touched is cancelled out
   				...
   				$color_flow->{_is_last_flow_index_touched_green} 	= $false;

							# print("color_flow delete_from_flow_button,
...
sub flow_select {
	...
	#TODO: careful, color is re-assigned from conditions_gui,few lines up ... necessary?
 	my $flow_color				= $color_flow ->{_flow_color}; 
 				# print("11. green_flow, flow_select,flow color: $color_flow->{_flow_color} = flow_color $flow_color \n");
	...
	if ($pre_req_ok) {
		...
		$color_flow->{_prog_name_sref} 	= $flow_widgets->get_current_program(\$_flow_listbox_color_w);
			...
			# Is a program deleted through a previous dragNdrop?
  			if( $flow_widgets->is_drag_deleted($_flow_listbox_color_w) ) {
			  				 ...
			  				 # print("\n\ncolor_flow flow_selectsomething was deleted in previous dragNdrop\n");
			  				 
        		my $this_index 		= $flow_widgets->get_drag_deleted_index($_flow_listbox_color_w);
        		
							 # print("color_flow flow_selectdeleting flow_listbox,idx=$thi# s_index\n");
		...
		# Assume the touched program had its parameter values modified
		# Update all the previously touched program's values in storage via param_flow
		
		# CASE 2
		# The current color is different to the previously touched color
		# LOSE all the previous changes

		# only if the last color-flow box touched was of the current flow color
		_check4flow_changes();
 		...
 		$color_flow->{_names_aref} 			= $param_flow_color_i->get_names_aref();
  		$color_flow->{_values_aref} 		= $param_flow_color_i->get_values_aref();
  							...
  							# print("2. color_flow flow_select,param_flow_green: $param_flow_green\, flow_color: $flow_color\n");  						
  				 			  ...
  				 			  # print("3. color_flow flow_selectparameter values:@{$color_flow->{_values_aref}}[1]\n");
  				 			 
 		$color_flow->{_check_buttons_settings_aref} 
									=  $param_flow_color_i->get_check_buttons_settings();

  				 			 # print("4. color_flow flow_select check_buttons_settings no changes, @{$color_flow->{_check_buttons_settings_aref}}\n");
  				 			 # print("4. color_flow flow_selectindex: $index \n");
		...
		$here 					= $whereami->get4flow_listbox();
				 			  ...
				 			  # print("5. color_flow flow_selecthere->{_is_flow_listbox_color_w: $here->{_is_flow_listbox_color_w}\n");
		...
		$param_widgets				->set_labels_w_aref($color_flow->{_labels_w_aref} );
 		...
 		$param_widgets				->set_check_buttons($color_flow->{_check_buttons_settings_aref});
					  		 ...
					  		 # print("6. color_flow flow_select is listbox selected:$here->{_is_flow_listbox_color_w}\n");
    	...
    	$prog_name_sref 	= $color_flow->{_prog_name_sref};
   		...
   		return();
  			...
  			# print("22. green_flow, flow_select,flow color: $color_flow->{_flow_color} = flow_color $flow_color \n");
}
 	...
 	sub set_hash_ref {
		...
		$message_w								= $hash_ref->{_message_w}; 
 		...
 		$values_w_aref 							= $hash_ref->{_values_w_aref};
 		 ...
 		 # print("color_flow,set_hash_ref,_flow_name_green_w $flow_name_green_w \n");
